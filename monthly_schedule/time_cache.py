...
produced them; on lookup the entry is invalidated (dropped) when:
  ...
  * the cached arrival no longer falls inside the current arrival window
    (member's availability changed, plan rules shifted, etc.).
...
def _entry_valid(entry, plan, travel_minutes, arrival_window):
    """Is the cached entry still authoritative under current rules?
    Checks plan + travel match and that the cached arrival is still
    inside the current arrival window."""
    if entry.get("plan") != plan:
        return False
    if entry.get("travel_minutes") != travel_minutes:
        return False
    try:
        arrival_min = parse_hhmm(entry["arrival"])
    except (KeyError, ValueError):
        return False
    lo, hi = arrival_window
    return lo <= arrival_min <= hi


def lookup_times(cache, center_id, day,
                  plan, travel_minutes, arrival_window):
    """Return the cached times dict for (center_id, day) if it is still
    valid under the current plan/travel/arrival_window. Otherwise return
    None and (as a side effect) drop the stale entry so save_time_cache
    won't preserve it."""
    ...
    if entry is None:
        return None
    if not _entry_valid(entry, plan, travel_minutes, arrival_window):
        del member[iso]
        if not member:
