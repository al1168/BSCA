...
def _seed_default_availability(cur, today, exclude_test, stats):
    ...
    weekday HHA narrowed to an earlier end-time keeps that narrower
    row (the OPEN-row check sees it and skips). Unmentioned weekdays
    get the default. The scheduler still gates per-day by
    Authorization.auth_days, so a default row on a non-authorized
    weekday is inert.
    """
