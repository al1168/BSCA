...
from scripts import add_auth_number_to_authorization as migration
...
def test_alter_statement_shape():
    q = migration._ALTER_ADD_AUTH_NUMBER
    assert "ALTER TABLE [Authorization]" in q
    assert "ADD COLUMN [auth_number]" in q
...
def test_column_exists_true_case_insensitive():
    cur = _FakeCursor(
        existing_columns=["Center ID", "auth_days", "auth_number"],
    )
    assert migration._column_exists(
        cur, "Authorization", "auth_number",
    ) is True
    assert migration._column_exists(
        cur, "Authorization", "AUTH_NUMBER",
    ) is True
...
def test_main_skips_when_column_already_exists(tmp_path, capsys, monkeypatch):
    fake_conn = _FakeConn(existing_columns=["Center ID", "auth_number"])
...
def test_main_adds_column_when_absent(tmp_path, capsys, monkeypatch):
    ...
    assert executed_sqls == [migration._ALTER_ADD_AUTH_NUMBER]
    assert fake_conn.committed is True
    out = capsys.readouterr().out
    assert "Added: Authorization.[auth_number]" in out
