...
from scripts import add_long_lat_to_contacts as migration
...
def test_alter_statement_shape():
    q = migration._ALTER_ADD_LONG_LAT
    assert "ALTER TABLE [Contacts]" in q
    assert "ADD COLUMN [Long Lat]" in q
...
class _FakeCursor:
    ...
    def columns(self, table):
        ...
        # `columns().fetchall()` is the call shape used in the script.
...
def test_column_exists_true_case_insensitive():
    cur = _FakeCursor(existing_columns=["Center ID", "Last Name", "Long Lat"])
    assert migration._column_exists(cur, "Contacts", "Long Lat") is True
    # Case-insensitive lookup.
    assert migration._column_exists(cur, "Contacts", "long lat") is True


def test_column_exists_false_when_absent():
    cur = _FakeCursor(existing_columns=["Center ID", "Last Name", "Address"])
    assert migration._column_exists(cur, "Contacts", "Long Lat") is False
...
def test_main_skips_when_column_already_exists(tmp_path, capsys, monkeypatch):
    fake_conn = _FakeConn(existing_columns=["Center ID", "Long Lat"])
...
def test_main_adds_column_when_absent(tmp_path, capsys, monkeypatch):
    fake_conn = _FakeConn(existing_columns=["Center ID", "Last Name"])
    monkeypatch.setattr("pyodbc.connect", lambda cs: fake_conn)
    db_file = tmp_path / "test.accdb"
    db_file.touch()
    ...
    assert executed_sqls == [migration._ALTER_ADD_LONG_LAT]
    assert fake_conn.committed is True
    out = capsys.readouterr().out
    assert "Added: Contacts.[Long Lat]" in out
