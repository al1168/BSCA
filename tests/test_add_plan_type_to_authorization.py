...
from scripts import add_plan_type_to_authorization as migration
...
def test_alter_statement_shape():
    q = migration._ALTER_ADD_PLAN_TYPE
    assert "ALTER TABLE [Authorization]" in q
    assert "ADD COLUMN [Plan Type]" in q
...
def test_column_exists_true_case_insensitive():
    cur = _FakeCursor(
        existing_columns=["Center ID", "auth_days", "Plan Type"],
    )
    assert migration._column_exists(
        cur, "Authorization", "Plan Type",
    ) is True
    assert migration._column_exists(
        cur, "Authorization", "plan type",
    ) is True
...
def test_main_skips_when_column_already_exists(tmp_path, capsys, monkeypatch):
    fake_conn = _FakeConn(existing_columns=["Center ID", "Plan Type"])
...
def test_main_adds_column_when_absent(tmp_path, capsys, monkeypatch):
    ...
    assert executed_sqls == [migration._ALTER_ADD_PLAN_TYPE]
    assert fake_conn.committed is True
    out = capsys.readouterr().out
    assert "Added: Authorization.[Plan Type]" in out
